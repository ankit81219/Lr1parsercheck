...
export default function (document) {
 ...
 //First logic
// const FIRST = {}; // global cache
// const nullableSymbols = new Set(); // global nullable tracker
...
// };
  ...
  const closure = set => {
    ...
    //console.log('initial closureSet JSON:\n', JSON.stringify(closureSet, null, 2));
    let newItemAdded = true;
    while (newItemAdded) {
      newItemAdded = false;
      for (let i = 0; i < closureSet.length; ++i) {

        const {rule,position,lookahead}=closureSet[i];

        const rhs=rule.rhs;
        let symbol = rhs[position];
        //console.log(symbol);
          ...
          if (_rules[j].lhs === symbol) {
            let f = first(
              closureSet[i].position + 1 < closureSet[i].rule.rhs.length ?
                ...
                //console.log(f);
            // let beta = closureSet[i].rule.rhs.slice(closureSet[i].position + 1);
            // beta.push(closureSet[i].lookahead); // βa
            // let f = firstSeq(beta); // firstSeq handles arrays of symbols
 ...
 const createCollection = () => {
  ...
  //console.log('📦 Initial state (I0):');
  //printState(0, _collection[0]);
      ...
      for (let symbol = 1; symbol < _symbols.length - 1; ++symbol) {
        ...
        if (set.length !== 0) {
          ...
          if (newSet) {
            ...
            newSetAdded = true;
            //console.log(`\n📦 New state (I${_collection.length - 1}) via symbol '${_symbols[symbol]}':`);
            //printState(_collection.length - 1, set);
          }
        }
      }
    }
...
const printState = (index, set) => {
  console.log(`\nState I${index}:`);
  for (let item of set) {
    const lhs = item.rule.lhs;
    const rhs = [...item.rule.rhs];
    rhs.splice(item.position, 0, '•'); // insert dot
    console.log(`  ${lhs} → ${rhs.join(' ')} , ${item.lookahead}`);
  }
};
